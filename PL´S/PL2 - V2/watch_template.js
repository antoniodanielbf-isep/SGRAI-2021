...
export default class Watch {
    constructor(cityName, center = new THREE.Vector3(0.0, 0.0, 0.0), radius = 0.75, nameBackgroundColor = 0xffffff, nameForegroundColor = 0x000000, dialColor = 0x000000, markersColor = 0xffffff, handsHMColor = 0xffffff, handSColor = 0xff0000) {
        ...
        /* To-do #2 - Create the sixty markers (sixty line segments) as follows:
            ...
            let xFora, yFora, xDentro, yDentro;

            for(let i = 0; i < 60; i++){
                xFora = radius2 * Math.cos(angle) + x0;
                yFora = radius2 * Math.sin(angle) + y0;
                if(i%5 == 0){
                    xDentro = radius0 * Math.cos(angle) + x0;
                    yDentro = radius0 * Math.sin(angle) + y0; 
                }else{
                    xDentro = radius1 * Math.cos(angle) + x0;
                    yDentro = radius1 * Math.sin(angle) + y0;
                }
            
                angle += increment;
                points.push( new THREE.Vector3( xFora, yFora, z0 ) );
                points.push( new THREE.Vector3( xDentro, yDentro, z0 ) );
            }
    ...
    update() {
        ...
        angle = Math.PI / 2.0 - 2.0 * Math.PI * ((time[0] / 12.0) + (time[1] / 12.0 * 60.0) + (time[2] / 12.0 * 60.0 * 60.0)) ;
